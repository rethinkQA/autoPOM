...
**Slice 3 polish: `pw-crawl auth-setup`** is a small interactive subcommand that codifies the manual storageState-capture flow. Opens a headed browser at `<url>`, waits for `ENTER` on stdin, then writes Playwright `storageState` JSON to `-o <file>`. Output is consumed by `--auth-state` on explore/drift. Replaces the `playwright codegen --save-storage` workflow with a one-shot CLI that doesn't require a second tool.

**Slice 5: agent-driven login via `fill_field` + credentials.** The agent gets a fifth tool — `fill_field({ locator, value, label })` — that maps to Playwright `.fill()` and MCP `browser_type`. Combined with `--credentials-file <path>` (a JSON of `{KEY: value}` pairs), the agent can log in itself: it emits `fill_field({ value: "{{EMAIL}}" })`, the loop substitutes the placeholder against the credentials map at dispatch time, and the actual credential string never appears in the agent's output. This is the canonical workaround for the MCP+auth limitation: the agent does the login flow itself, cookies get set via the real form submission, and `--mcp` works because no `storageState` injection is involved.

**Known limitation: `--mcp` + `--auth-state` together.** Multiple attempts at sharing an authenticated context across our Playwright handle and `@playwright/mcp` did not succeed: cookies injected into the CDP-shared default context (the one MCP picks via `contexts()[0]`) don't reach the page MCP drives, and `newContext({ storageState })` creates a context MCP doesn't see as `contexts()[0]`. The CLI now detects the combination, prints a warning, and silently falls back to the direct-Playwright path for that run — which applies `storageState` reliably. Real fix needs deeper investigation of `@playwright/mcp`'s context-resolution semantics; deferred.
